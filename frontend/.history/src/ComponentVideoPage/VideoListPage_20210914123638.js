...
const VideoListPage = (props) => {
    ...
    console.log(styles['video-category-title']);
    useEffect(()=>{
        if(props.isShownAddVideoForm) {
            document.querySelector('.VideoListPage_video-category-title__LLSL9').style.marginTop='87px';
        } else {
            document.querySelector('.VideoListPage_video-category-title__LLSL9').style.marginTop='120px';
        }

    }, [props.isShownAddVideoForm])
    ...
    return (
        <div className={styles['video-category-page']}>
            ...
            <button type="button" className={addVideoBtnClassName} onClick={props.onShowAddVideoForm}>Thêm video</button>
